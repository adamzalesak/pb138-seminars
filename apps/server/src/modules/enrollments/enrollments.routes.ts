...
export const enrollmentsRouter = new Elysia({ prefix: '/students', tags: ['Enrollments'] })
  ...
  .post(
    '/:id/enroll',
    async ({ params: { id: studentId }, body }) => {
    ...
    },
    {
      ...
      detail: {
        description:
          'Enroll a student into multiple courses at once. Uses a transaction — all or nothing.',
      },
    },
  )
  ...
  .post(
    '/:id/transfer',
    async ({ params: { id: studentId }, body, set }) => {
      try {
        ...
        if (
          message === 'Target course is full' ||
          message === 'Student is not enrolled in the source course' ||
    ...
    },
    {
      ...
      detail: {
        description:
          'Transfer a student from one course to another. Uses a serializable transaction to enforce capacity limits.',
      },
    },
  )
